...
$.ajax({
	type: 'get',
	url: 'http://localhost:8080/KnowLife/GetUserUploadVideoServlet',
	dataType: 'JSON',
	...
	success: function(data) {
		...
		for (var i = 0; i < length; i++) {
			...
			$("#videoBody").append(`<tr>
							<td>${jsonId.id}</td>
							<td><div class="container"><img src='images/${jsonId.imgFileName}'class="img-fluid" style="height:50px;"></div></td>
							<td><div class="container"><p>${jsonId.videoName}</p></div></td>
							<td>
						<!-- 影片修改 -->
						<div class="">
									...
									<div class="row">
                               		...
                               		<div class="col-sm-12 col-md-6">
										<form action="/KnowLife/videoNameUpdate/" method="post" enctype="application/json">
											...
											<input type="text" class="form-control" id="${jsonId.id + "name"}" aria-describedby="emailHelp" name="videoName" value="${jsonId.videoName}">									
											<input type="hidden" class="form-control" id="${jsonId.id + "name"}" aria-describedby="emailHelp" name="videoId" value="${jsonId.id}">
									...
									<div class="col-sm-12 col-md-6">
										<form action="/KnowLife/videoPriceUpdate/" method="post" enctype="application/json">
                                			<div class="col-sm-6">
												...
												<input type="text" class="form-control" id="${jsonId.id + "name"}" aria-describedby="emailHelp" name="videoPrice" value="${jsonId.videoPrice}">									
												<input type="hidden" class="form-control" id="${jsonId.id + "name"}" aria-describedby="emailHelp" name="videoId" value="${jsonId.id}">
                       							<button type="submit" class="btn btn-primary">送出</button>
										</form>
                               		</div>
						...
						<form action="/KnowLife/videoFileUpdate/" method="post" enctype="multipart/form-data">
							...
							<td><a class="btn btn-sm btn-secondary"
								href="/KnowLife/videoDelete/${jsonId.id}"><i
